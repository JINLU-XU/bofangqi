    ...
    <div>
      ...
      <div class="s_lists">
        <div class="song" v-for="(item,index) in lists" :key="index" @click="song(index)">
          <div class="lists_img">
            <img :src="item.data.djRadio.dj.avatarUrl" alt />
            <div class="mask">
              <span class="iconfont icon-bofang"></span>
            </div>
          </div>
          <div class="descript">
            <span>{{item.data.djRadio.desc}}</span>
          </div>
          <!-- <div class="playCount">
...
import nosong from "../kgorder/nosong.vue"
    ...
    export default {
        ...
        created(){
               this.id = this.$store.state.song3;
            if(this.id.length>0){
                this.isvalue = false
            }
            
            for(let i=0;i<this.id.length;i++){
                 this.$axios.get("/dj/detail?rid="+this.id[i]+"").then(res => {
                     this.lists.push(res)
        ...
        methods:{
             song(index){
          this.$store.commit("song/addDJ",this.id[index])
      this.$router.push("/dj")
